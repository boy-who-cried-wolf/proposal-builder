...
export function ExpandableTabs({
  ...
  return <div className="flex flex-col">
      <div ref={outsideClickRef} className={cn("flex flex-wrap items-center gap-2 bg-background p-1", className)}>
        {tabs.map((tab, index) => {
        ...
        return <motion.button key={`tab-${index}-${(tab as Tab).title}`} variants={buttonVariants} initial="initial" animate="animate" custom={isExpanded} onClick={e => {
          e.preventDefault();
          e.stopPropagation();
          handleSelect(index);
        }} onMouseEnter={() => setHovered(index)} onMouseLeave={() => setHovered(null)} transition={transition} className="my-0 py-[5px] px-[15px]">
              <TabIcon size={20} />
              <AnimatePresence initial={false}>
                {isExpanded && <motion.span variants={spanVariants} initial="initial" animate="animate" exit="exit" transition={transition} className="overflow-hidden whitespace-nowrap">
                    {(tab as Tab).title}
                  </motion.span>}
              </AnimatePresence>
            </motion.button>;
      ...
      {showTabContent && selected !== null && <div className="mt-4 p-4 bg-muted/20 rounded-lg">
